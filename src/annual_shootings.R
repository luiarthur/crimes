...
for (i in 1:length(all_years)) {
  ...
  school[i] = sum(tolower(as.character(shoot$Venue[idx])) == "school")
}

annual_shootings = as.data.frame(cbind('year'=all_years, 'count'=counts_years, auto, legal, ill, white, school))
...
lines(annual_shootings$year, annual_shootings$school, col='pink', pch=20, lwd=2)
